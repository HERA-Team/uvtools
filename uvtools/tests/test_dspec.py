...
def test_wedge_width():
    ...
    assert dspec.wedge_width(0, 0.01, 10) == (1, 10)
    assert dspec.wedge_width(5.0, 0.01, 10) == (1, 10)
    assert dspec.wedge_width(9.0, 0.01, 10) == (2, -1)
    assert dspec.wedge_width(10.0, 0.01, 10) == (2, -1)
    assert dspec.wedge_width(15.0, 0.01, 10) == (3, -2)
    # test nchan
    assert dspec.wedge_width(10.0, 0.01, 20) == (3, -2)
    assert dspec.wedge_width(10.0, 0.01, 40) == (5, -4)
    # test sdf
    assert dspec.wedge_width(10.0, 0.02, 10) == (3, -2)
    assert dspec.wedge_width(10.0, 0.04, 10) == (5, -4)
    # test standoff
    assert dspec.wedge_width(100.0, 0.001, 100, standoff=4.0) == (11, -10)
    assert dspec.wedge_width(100.0, 0.001, 100, standoff=5.0) == (11, -10)
    assert dspec.wedge_width(100.0, 0.001, 100, standoff=10.0) == (12, -11)
    assert dspec.wedge_width(100.0, 0.001, 100, standoff=15.0) == (13, -12)
    # test horizon
    assert dspec.wedge_width(100.0, 0.001, 100, horizon=0.1) == (2, -1)
    assert dspec.wedge_width(100.0, 0.001, 100, horizon=0.5) == (6, -5)
    assert dspec.wedge_width(100.0, 0.001, 100, horizon=1.5) == (16, -15)
    assert dspec.wedge_width(100.0, 0.001, 100, horizon=2.0) == (21, -20)
...
def test_delay_filter_1D():
    ...
    wgts = 0.5 * np.ones(NCHAN, dtype=complex)
    dmdl, dres, info = dspec.delay_filter(data, wgts, 0.0, 0.1 / NCHAN, tol=TOL)
    ...
    dmdl, dres, info = dspec.delay_filter(
        data, wgts, 0.0, 0.1 / NCHAN, tol=TOL, skip_wgt=0.1
    )
    np.testing.assert_allclose(data, dmdl, atol=NCHAN * TOL)
    np.testing.assert_allclose(dres, np.zeros_like(dres), atol=NCHAN * TOL)
    data = np.random.normal(size=NCHAN)
    wgts = np.ones_like(data)
    dmdl, dres, info = dspec.delay_filter(data, wgts, 0.0, 0.1 / NCHAN, tol=1e-9)
    ...
    wgts[:72] = 0.0
    dmdl, dres, info = dspec.delay_filter(
        data, wgts, 0.0, 0.1 / NCHAN, tol=TOL, skip_wgt=0.5, mode="clean"
    )
...
def test_delay_filter_2D():
    ...
    np.testing.assert_allclose(dres, np.zeros_like(dres), atol=NCHAN * TOL)
    wgts[:, ::16] = 0
    wgts *= 0.9  # tests to make sure wgts**2 normalization works
    dmdl, dres, info = dspec.delay_filter(data, wgts, 0.0, 0.1 / NCHAN, tol=TOL)
    np.testing.assert_allclose(data, dmdl, atol=NCHAN * TOL)
    np.testing.assert_allclose(dres, np.zeros_like(dres), atol=NCHAN * TOL)
    ...
    np.testing.assert_allclose(
        np.average(data, axis=1), np.average(dmdl, axis=1), atol=1e-3
    )
...
def test_fourier_model():
    ...
    NFREQS = 100
    nmodes = 2 * NMAX + 1
    cn = (np.arange(nmodes) + 1.0j * np.arange(nmodes)) / float(nmodes)
...
def test_delay_filter_leastsq():
    ...
    bf_model, cn_out, data_out = dspec.delay_filter_leastsq(
        data, flags, sigma, nmax=3, add_noise=False
    )
    ...
    pytest.raises(
        ValueError,
        dspec.delay_filter_leastsq_1d,
        data[0],
        flags[0],
        sigma,
        nmax=3,
        operator=np.empty((3, 3)),
    )
    pytest.raises(
        ValueError,
        dspec.delay_filter_leastsq_1d,
...
def test_skip_wgt():
    ...
    np.testing.assert_allclose(
        dres[1:, :], np.zeros_like(dres)[1:, :], atol=NCHAN * TOL
    )
    ...
    assert np.all(
        [
            ...
            for i in list(info["status"]["axis_1"])[:1]
        ]
    )
    assert not np.any(
        [
...
def test_gen_window():
    for w in [
        ...
        assert np.all(np.isclose(win, win3 * np.sqrt(np.mean(win**2.0)), atol=1e-6))

    pytest.raises(ValueError, dspec.gen_window, "foo", 200)
    # check Ncut ValueError
    pytest.raises(
        ValueError, dspec.gen_window, "bh", 200, edgecut_hi=101, edgecut_low=100
    )
...
def test_dft_operator():
    ...
    fop = dspec.dft_operator(freqs, 0.0, 1e-6)
    fg, dg = np.meshgrid(
        freqs - 150e6, np.arange(-10, 10) * (1.0 / DF / NF), indexing="ij"
    )
    y = np.exp(2j * np.pi * fg * dg)
    np.testing.assert_allclose(fop, y)
    fg, dg = np.meshgrid(
        freqs - 150e6, np.arange(-20, 20) * (1.0 / DF / NF / 2), indexing="ij"
    )
    # check fundamental period x 2 works alright
    # and gives us expected values
    y1 = np.exp(2j * np.pi * fg * dg)
    fop1 = dspec.dft_operator(freqs, 0.0, 1e-6, fundamental_period=200 * 1e5)
...
def test_dpss_operator():
    ...
    freqs = np.arange(-NF / 2, NF / 2) * DF + 150e6
    freqs_bad = np.array(
        [1.100912386458, 1.22317, 2.12341260, 3.234632462, 5.32348356887]
    )
    pytest.raises(
        ValueError,
        dspec.dpss_operator,
    ...
    amat1, ncol1 = dspec.dpss_operator(freqs, [0.0], [100e-9], eigenval_cutoff=[1e-9])
    amat2, ncol2 = dspec.dpss_operator(freqs, [0.0], [100e-9], edge_suppression=[1e-9])
    amat3, ncol3 = dspec.dpss_operator(freqs, [0.0], [100e-9], avg_suppression=[1e-9])
    ncols = [ncol1, ncol2, ncol3]
    ncolmin = np.min(ncols)
    ncolmax = np.max(ncols)
    amat4, ncol4 = dspec.dpss_operator(freqs, [0.0], [100e-9], nterms=[ncolmax])
...
def test_fit_solution_matrix():
    ...
    data = np.exp(2j * np.pi * 3.5 / 50.0 * fs) + 5 * np.exp(
        2j * np.pi * 2.1 / 50.0 * fs
    )
    data += np.exp(-2j * np.pi * 0.7 / 50.0 * fs) + 5 * np.exp(
        2j * np.pi * -1.36 / 50.0 * fs
    )
    ...
    amat_dft = dspec.dft_operator(fs, [0.0], [4.0 / 50.0], fundamental_period=140.0)
    amat_dpss, _ = dspec.dpss_operator(fs, [0.0], [5.0 / 50.0], eigenval_cutoff=[1e-15])
    wmat = np.diag(wgts)
    ...
    amat_dft_pc = dspec.dft_operator(fs, [0.0], [4.0 / 50.0], fundamental_period=200.0)
...
def test_dayenu_filter():
    ...
    # generate some noise
    noise = (np.random.randn(nf) + 1j * np.random.randn(nf)) / np.sqrt(2.0)
    ...
    dspec.dayenu_filter(
        ...
        1,
        filter_centers[0],
        filter_half_widths[0],
        filter_factors[0],
    )
    filter_half_widths2 = [200e-9, 200e-9]
    filter_centers2 = [0.0, -1400e-9]
    ...
    filtered_noise, _ = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * df,
        data_1d,
        wghts_1d,
        [1],
    ...
    np.testing.assert_almost_equal(
        np.std(filtered_noise.real) ** 2.0 + np.std(filtered_noise.imag) ** 2.0,
        np.std(noise.real) ** 2.0 + np.std(noise.imag) ** 2.0,
        decimal=0,
    )
    # now filter foregrounds and signal and test that std of residuals are close to std of signal.
    filtered_signal, _ = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * df,
        fg_sg,
        wghts_1d,
        [1],
    ...
    np.testing.assert_almost_equal(
        (np.std(filtered_signal.real) ** 2.0 + np.std(filtered_signal.imag) ** 2.0)
        / 1e4,
        (np.std(sg_tone.real) ** 2.0 + np.std(sg_tone.imag) ** 2.0) / 1e4,
        decimal=0,
    )

    # check that zeros where wgts are zero.
    wghts_1d[len(wghts_1d) // 4] = 0.0
    wghts_1d[len(wghts_1d) // 4 + 5] = 0.0
    filtered_noise, _ = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * df,
        data_1d,
        wghts_1d,
    ...
    assert np.all(filtered_noise[~(wghts_1d.astype(bool))] == 0.0)
    ...
    TEST_CACHE = {}
    OMEGA_EARTH = 2.0 * np.pi / 3600.0 / 24.0
    ...
    signal_2d = 1e6 * np.exp(2j * np.pi * 50.0 / 3e8 * np.sin(OMEGA_EARTH * tg) * fg)
    noise_2d = np.random.randn(nf, nf) / np.sqrt(2.0) + 1j * np.random.randn(
        nf, nf
    ) / np.sqrt(2.0)
    ...
    pytest.raises(
        ...
        x=np.arange(-nf / 2, nf / 2) * df,
        data=data_2d,
        wgts=np.ones_like(data_2d),
        filter_dimensions=[1, 0],
        filter_centers=[[0.0], [0.0]],
        filter_half_widths=[[1e-3], [100e-9]],
    ...
    pytest.raises(
        ValueError,
        dspec.dayenu_filter,
        x=["time is a construct", np.arange(-nf / 2, nf / 2) * df],
        data=data_2d,
        wgts=np.ones_like(data_2d),
        filter_dimensions=[1, 0],
        filter_centers=[[0.0], [0.0]],
    ...
    pytest.raises(
        ValueError,
        dspec.dayenu_filter,
        x=[times, np.arange(-nf / 2, nf / 2) * df],
        data=data_2d,
        wgts=np.ones_like(data_2d),
        filter_dimensions=[1, 0],
        filter_centers=[[0.0], [0.0]],
    ...
    filtered_data_fr, _ = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * dt,
        data_2d,
        np.ones_like(data_2d),
        filter_centers=[0.0],
        filter_half_widths=[0.004],
        filter_factors=[1e-10],
        filter_dimensions=[0],
        cache=TEST_CACHE,
    )

    np.testing.assert_almost_equal(
        np.sqrt(np.mean(np.abs(filtered_data_fr.flatten()) ** 2.0)), 1.0, decimal=1
    )

    # only filter in the delay-domain.

    filtered_data_df, _ = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * df,
        data_2d,
        np.ones_like(data_2d),
        filter_centers=[0.0],
    ...
    np.testing.assert_almost_equal(
        np.sqrt(np.mean(np.abs(filtered_data_df.flatten()) ** 2.0)), 1.0, decimal=1
    )
    ...
    filtered_data_df_fr, _ = dspec.dayenu_filter(
        [np.arange(-nf / 2, nf / 2) * dt, np.arange(-nf / 2, nf / 2) * df],
        data_2d,
        np.ones_like(data_2d),
        filter_centers=[[0.002], [0.0]],
        filter_half_widths=[[0.001], [100e-9]],
        filter_factors=[[1e-5], [1e-5]],
        filter_dimensions=[0, 1],
        cache=TEST_CACHE,
    )
    ...
    np.testing.assert_almost_equal(
        np.sqrt(np.mean(np.abs(filtered_data_df_fr.flatten()) ** 2.0)), 1.0, decimal=1
    )
    ...
    # test skip_wgt:
    _, info = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * df,
        data_1d,
        np.zeros_like(wghts_1d),
        [1],
        np.array(filter_centers),
        np.array(filter_half_widths),
        np.array(filter_factors),
    ...
    assert np.all(
        [info["status"]["axis_1"][i] == "skipped" for i in info["status"]["axis_1"]]
    )

    _, info = dspec.dayenu_filter(
        np.arange(-nf / 2, nf / 2) * df,
        data_1d,
        np.ones_like(wghts_1d),
        [1],
        np.array(filter_centers),
        np.array(filter_half_widths),
        np.array(filter_factors),
    ...
    assert np.all(
        [info["status"]["axis_1"][i] == "success" for i in info["status"]["axis_1"]]
    )


def test_dayenu_mat_inv():
    freqs = np.arange(-16, 16) * 100e3
    cmat = dspec.dayenu_mat_inv(
        freqs, filter_centers=[], filter_half_widths=[], filter_factors=[]
    )
    ...
    cmat1 = dspec.dayenu_mat_inv(
        freqs, filter_centers=0.0, filter_half_widths=112e-9, filter_factors=1e-9
    )
    cmat2 = dspec.dayenu_mat_inv(
        freqs, filter_centers=[0.0], filter_half_widths=[112e-9], filter_factors=[1e-9]
    )
    x, y = np.meshgrid(np.arange(-16, 16), np.arange(-16, 16))
    cmata = np.identity(32).astype(np.complex128) + 1e9 * np.sinc(
        (x - y) * 100e3 * 224e-9
...
def test_vis_filter():
    ...
    uvd = UVData()
    uvd.read_miriad(
        os.path.join(DATA_PATH, "zen.2458042.17772.xx.HH.uvXA"), bls=[(24, 25)]
    )
    ...
    d = 200 * np.exp(
        -2j * np.pi * times[:, None] * (frs[2] + dfr / 4)
        - 2j * np.pi * freqs[None, :] * (dlys[2] + ddly / 4) / 1e9
    )
    d += 50 * np.exp(
        -2j * np.pi * times[:, None] * (frs[20])
        - 2j * np.pi * freqs[None, :] * (dlys[20]) / 1e9
    )
    d += 10 * (
        (
            np.random.normal(0, 1, uvd.Nfreqs * uvd.Ntimes).astype(complex)
            + 1j * np.random.normal(0, 1, uvd.Nfreqs * uvd.Ntimes)
        ).reshape(uvd.Ntimes, uvd.Nfreqs)
    )
    ...
    mdl, res, info = dspec.delay_filter(
        d,
        w,
        sdf,
        bl_len,
    ...
    mdl, res, info = dspec.delay_filter(
        d, w, sdf, bl_len, tol=1e-4, window="none", skip_wgt=0.1, gain=0.1
    )
    ...
    mdl2, res2, info2 = dspec.vis_filter(
        d,
        w,
        bl_len=bl_len,
        sdf=sdf,
    ...
    mdl, res, info = dspec.fringe_filter(
        d, w, frs[15], dt, tol=1e-4, window="none", skip_wgt=0.1, gain=0.1
    )
    cln = mdl + res
    ...
    # test vis filter is the same
    mdl2, res2, info2 = dspec.vis_filter(
        d, w, max_frate=frs[15], dt=dt, tol=1e-4, window="none", skip_wgt=0.1, gain=0.1
    )
    cln2 = mdl2 + res2
    assert np.isclose(mdl - mdl2, 0.0).all()
    ...
    mdl, res, info = dspec.fringe_filter(
        d, w, (frs[-20], frs[10]), dt, tol=1e-4, window="none", skip_wgt=0.1, gain=0.1
    )
    cln = mdl + res
    ...
    mdl2, res2, info = dspec.vis_filter(
        d,
        w,
        bl_len=bl_len,
        sdf=sdf,
...
def test_delay_interpolation_matrix():
    ...
    fs = np.arange(-10, 10)
    # here is some underlying data
    data = np.exp(2j * np.pi * 3 / 20.0 * fs) + 5 * np.exp(2j * np.pi * 4.0 / 20 * fs)
    data += np.exp(-2j * np.pi * 3 / 20.0 * fs) + 5 * np.exp(
        2j * np.pi * -4 / 20.0 * fs
    )
    # here are some weights with flags
    wgts = np.ones_like(data)
    ...
    data_interp = np.dot(
        dspec.delay_interpolation_matrix(
            nchan=20, ndelay=5, wgts=wgts, fundamental_period=20, cache=MYCACHE
        ),
        dw,
    )
    ...
    data_interp1 = dspec.delay_interpolation_matrix(
        nchan=20, ndelay=5, wgts=wgts, fundamental_period=20, cache={}
    )
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        wgtpc[randflags] = 0.0
        amat_pc = dspec.delay_interpolation_matrix(
            nchan=100, ndelay=25, wgts=wgtpc, fundamental_period=200
        )
        print(len(w))
        assert len(w) > 0
...
def test_fourier_filter():
    ...
    dfr, ddly = frs[1] - frs[0], dlys[1] - dlys[0]
    d = 200 * np.exp(
        -2j * np.pi * times[:, None] * (frs[ntimes // 2 + 2] + dfr / 4)
        - 2j * np.pi * freqs[None, :] * (dlys[nf // 2 + 2] + ddly / 4)
    )
    d += 50 * np.exp(
        -2j * np.pi * times[:, None] * (frs[ntimes // 2 - 3])
        - 2j * np.pi * freqs[None, :] * (dlys[nf // 2 + 3])
    )
    ...
    dft_options1 = {"fundamental_period": 2.0 * (times.max() - times.min())}
    dft_options2 = {"fundamental_period": 2.0 * (times.max() - times.min())}
    dft_options3 = {"fundamental_period": 2.0 * (dlys.max() - dlys.min())}
    clean_options1 = {
        "tol": 1e-9,
        "maxiter": 100,
        "filt2d_mode": "rect",
    ...
    mdl1, res1, info1 = dspec.fourier_filter(
        x=freqs,
        data=d,
        wgts=w,
        filter_centers=[0.0],
    ...
    mdl2, res2, info2 = dspec.fourier_filter(
        freqs,
        d,
        w,
        [0.0],
    ...
    mdl3, res3, info3 = dspec.fourier_filter(freqs, d, w, [0.0], [bl_len], mode="clean")

    mdl4, res4, info4 = dspec.fourier_filter(
        freqs, d, w, [0.0], [bl_len], mode="clean", **clean_options1
    )
    ...
    mdl3, res3, info3 = dspec.fourier_filter(
        freqs, d, w, [0.0], [bl_len], suppression_factors=[1e-9], mode="dayenu"
    )
    ...
    mdl3, res3, info3 = dspec.fourier_filter(
        freqs, np.zeros_like(d), w, [0.0], [bl_len], mode="clean", filter_dims=1
    )
    assert np.all(
        [info3["status"]["axis_1"][i] == "skipped" for i in info3["status"]["axis_1"]]
    )
    ...
    mdl11d, res11d, info11d = dspec.fourier_filter(
        x=freqs,
        data=d[0],
        wgts=w[0],
        filter_centers=[0.0],
    ...
    mdl5, res5, info5 = dspec.fourier_filter(
        x=times,
        data=d,
        wgts=w,
        filter_centers=[0.0],
        filter_half_widths=[fr_len],
        suppression_factors=[0.0],
        filter_dims=0,
        mode="dpss_leastsq",
        **dpss_options1
    ...
    assert np.isclose(
        info["filter_params"]["axis_1"]["basis_options"]["fundamental_period"],
    ...
    mdl, res, info = dspec.fourier_filter(
        ...
        suppression_factors=[1e-9],
        mode="dft_leastsq",
        fundamental_period=4.0 * (freqs.max() - freqs.min()),
    )
    # check that the filter_period is indeed equal to 1 / (2 * bandwidth)
    assert np.isclose(
        info["filter_params"]["axis_1"]["basis_options"]["fundamental_period"],
        4.0 * (freqs.max() - freqs.min()),
    )

    # check fringe rate filter with dft mode
    mdl6, res6, info6 = dspec.fourier_filter(
        x=times,
        data=d,
        wgts=w,
        filter_centers=[0.0],
    ...
    mdl62, res62, info62 = dspec.fourier_filter(
        x=times,
        data=d,
        wgts=w,
        filter_centers=[0.0],
    ...
    # Check Dayenu filter.
    mdl7, res7, info7 = dspec.fourier_filter(
        x=times,
        data=d,
        wgts=w,
        filter_centers=[0.0],
        filter_half_widths=[fr_len],
        suppression_factors=[1e-8],
        filter_dims=0,
        mode="dayenu_dft_leastsq",
        **dft_options1
    )

    mdl8, res8, info8 = dspec.fourier_filter(
        x=times,
        data=d,
        wgts=w,
        filter_centers=[0.0],
    ...
    dft_options2_2d = {
        "fundamental_period": [
            4 * (times.max() - times.min()),
            4 * (freqs.max() - freqs.min()),
        ]
    }

    mdl9, res9, info9 = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=w,
        filter_centers=[[0.0], [0.0]],
        filter_half_widths=[[fr_len], [bl_len]],
        suppression_factors=[[1e-8], [1e-8]],
        mode="dayenu_dpss_leastsq",
        filter_dims=[1, 0],
        **dpss_options1_2d
    )
    ...
    mdl10, res10, info10 = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=w,
        filter_centers=[[0.0], [0.0]],
    ...
    pytest.raises(
        ...
        filter_centers=[[0.0], [0.0]],
        filter_half_widths=[[fr_len], [bl_len]],
        suppression_factors=[[0.0], [0.0]],
        mode="dft_leastsq",
        filter_dims=[1, 0],
        **dpss_options1
    )
    mdl_dft, res_dft, info_dft = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=w,
        filter_centers=[[0.0], [0.0]],
    ...
    mdl_dft1, res_dft1, info_dft1 = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=w,
        filter_centers=[[0.0], [0.0]],
    ...
    mdl11, res11, info11 = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=w,
        filter_centers=[[0.0], [0.0]],
        ...
        mode="clean",
        filter_dims=[1, 0],
        **{"filt2d_mode": "rect", "tol": 1e-5}
    )
    # try out plus mode. IDK
    mdl12, res12, info12 = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=w,
        filter_centers=[[0.0], [0.0]],
        ...
        mode="clean",
        filter_dims=[1, 0],
        **{"filt2d_mode": "plus", "tol": 1e-5}
    )

    # make sure that clean is skipped if all weights are zero.
    mdl13, res13, info13 = dspec.fourier_filter(
        x=[times, freqs],
        data=d,
        wgts=np.zeros_like(w),
        filter_centers=[[0.0], [0.0]],
        filter_half_widths=[[fr_len], [bl_len]],
        mode="clean",
        filter_dims=[1, 0],
    ...
    assert np.isclose(
        info_dft["filter_params"]["axis_1"]["basis_options"]["fundamental_period"],
        dft_options2_2d["fundamental_period"][1],
    )
    assert np.isclose(
        info_dft["filter_params"]["axis_0"]["basis_options"]["fundamental_period"],
...
def test_vis_clean():
    ...
    mdl1, res1, info1 = dspec.fourier_filter(
        freqs, d, w, [0.0], [bl_len], mode="clean", **{"tol": 1e-4}
    )

    # here is the vis filter implementation of clean
    # delay filter basic execution
    mdl2, res2, info2 = dspec.wedge_filter(
        d,
        w,
        bl_len,
        sdf,
    ...
    mdl3, res3, info3 = dspec.vis_clean(
        d,
        w,
        filter_size=bl_len,
        real_delta=sdf,
    ...
    mdl1, res1, info1 = dspec.fourier_filter(
        ...
        [bl_len],
        mode="clean",
        **{"tol": 1e-4, "window": "tukey", "edgecut_low": 4, "edgecut_hi": 4}
    )
    mdl2, res2, info2 = dspec.delay_filter(
        d,
        w,
        bl_len,
        sdf,
    ...
    mdl1, res1, info1 = dspec.fourier_filter(
        ...
        mode="clean",
        filter_dims=[0],
        **{"tol": 1e-4, "window": "tukey", "edgecut_low": 3, "edgecut_hi": 4}
    )
    mdl2, res2, info2 = dspec.fringe_filter(
        d,
        w,
        frs[15],
        dt,
    ...
    mdl2, res2, info2 = dspec.high_pass_fourier_filter(
        data=d,
        wgts=w,
        filter_size=[frs[15], bl_len],
    ...
    mdl3, res3, info3 = dspec.high_pass_fourier_filter(
        data=d,
        wgts=w,
        filter_size=[[frs[15], frs[15]], bl_len],
        real_delta=[np.mean(np.diff(times)), np.mean(np.diff(freqs))],
        window="tukey",
        tol=1e-5,
        clean2d=True,
    ...
    pytest.raises(
        ValueError,
        dspec.high_pass_fourier_filter,
        data=d,
        wgts=w,
        filter_size=[[frs[15], frs[15]], bl_len],
...
def test__fit_basis_1d():
    ...
    mod1, resid1, info1 = dspec._fit_basis_1d(
        fs,
        dw,
        wgts,
        [0.0],
        [5.0 / 50.0],
        basis_options=dpss_opts,
        method="leastsq",
        basis="dpss",
    )
    mod2, resid2, info2 = dspec._fit_basis_1d(
        fs,
        dw,
        wgts,
        [0.0],
    ...
    dft_opts = {"fundamental_period": 200.0}

    mod4, resid4, info4 = dspec._fit_basis_1d(
        fs,
        dw,
        wgts,
        [0.0],
        [5.0 / 50.0],
        basis_options=dft_opts,
        method="leastsq",
        basis="dft",
    )
    dft_opts = {"fundamental_period": 140.0}
    ...
    mod3, resid3, info3 = dspec._fit_basis_1d(
        fs,
        dw,
        wgts,
        [0.0],
...
def test_fit_basis_1d_with_missing_channels():
    ...
    mod5, resid5, info5 = dspec._fit_basis_1d(
        fs[to_keep],
        dw[to_keep],
        wgts[to_keep],
        [0.0, 10 / 50.0],
    ...
    mod6, resid6, info6 = dspec._fit_basis_1d(
        fs,
        dwt,
        wgtst,
        [0.0, 10 / 50.0],
...
def test_fit_basis_2d_nanwarning():
    ...
    dpss_opts = {"eigenval_cutoff": [1e-12, 1e-12]}
    with pytest.warns(RuntimeWarning):
        model, resid, info = dspec._fit_basis_2d(
            fs,
            data,
            wgts,
            [0.0, 10 / 50.0],
        ...
        assert list(
            np.where([info["status"]["axis_1"][i] == "skipped" for i in range(93)])[0]
        ) == [68]
    ...
    with pytest.warns(RuntimeWarning):
        ...
        assert list(
            np.where([info["status"]["axis_0"][i] == "skipped" for i in range(100)])[0]
        ) == [12]
