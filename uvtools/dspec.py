...
DAYENU_DEFAULTS_1D = {"suppression_factors": [1e-9], "max_contiguous_edge_flags": 10}
DPSS_DEFAULTS_1D = {
...
def wedge_width(bl_len, sdf, nchan, standoff=0.0, horizon=1.0):
...
def place_data_on_uniform_grid(x, data, weights, xtol=1e-3):
    ...
    if not np.allclose(
        xdiff / dx, np.round(xdiff / dx), rtol=0.0, atol=np.abs(xtol * dx)
    ):
        xout = x
        dout = data
        wout = weights
...
def _fourier_filter_hash(
    ...
    **kwargs,
    ...
    filter_key = (
        ...
        + tuple(
            np.round(
                np.asarray(filter_centers) * np.mean(np.diff(x)) * len(x), hash_decimal
            )
        )
    ...
    if w is not None:
        filter_key = (
            filter_key + ("weights",) + tuple(np.round(w.tolist(), hash_decimal))
        )
...
def fourier_filter(
    ...
    **filter_kwargs,
    ...
    if not mode in supported_modes:
        raise ValueError(
            "Need to supply a mode in supported modes:%s" % (str(supported_modes))
        )
    mode = mode.split("_")
    ndim_data = data.ndim
    ndim_wgts = wgts.ndim
    ...
    if not ndim_wgts == ndim_data:
        raise ValueError(
            "Number of dimensions in weights, %d does not equal number of dimensions in data, %d!"
            % (ndim_wgts, ndim_data)
        )
    ...
    if mode[0] == "dayenu":
        ...
        residual, info = dayenu_filter(
            x=x,
            data=data,
            wgts=wgts,
            filter_dimensions=filter_dims_d,
            filter_centers=filter_centers,
            filter_half_widths=filter_half_widths,
            filter_factors=suppression_factors,
            cache=cache,
            skip_wgt=skip_wgt,
        ...
        model = data - residual
        if len(mode) > 1:
            model, _, info_deconv = _fit_basis_2d(
                x=x,
                data=model,
                filter_centers=filter_centers,
                filter_dims=filter_dims_d,
                skip_wgt=skip_wgt,
                basis=mode[1],
                method=mode[2],
                wgts=wgts,
                basis_options=filter_kwargs,
                filter_half_widths=filter_half_widths,
        ...
        model, residual, info = _fit_basis_2d(
            x=x,
            data=data,
            filter_centers=filter_centers,
            filter_dims=filter_dims_d,
        ...
        model, residual, info = _clean_filter(
            x=x,
            data=data,
            wgts=wgts,
            filter_centers=filter_centers,
    ...
    if 0 in filter_dims and not filter2d:
        ...
        if "info_deconv" in info:
            for k in info["info_deconv"]:
                info["info_deconv"][k]["axis_0"] = copy.deepcopy(
...
def vis_clean(
    ...
    if clean2d:
        ...
        if not isinstance(real_delta, (list, tuple)) or not len(real_delta) == 2:
            raise ValueError(
                "real_delta must be length 2 list or tuple if clean2d is True."
            )
        x = [
            np.arange(data.shape[0]) * real_delta[0],
        ...
        for m in range(2):
            if isinstance(filter_size[m], (list, tuple)):
                fc.append([(filter_size[m][1] - filter_size[m][0]) / 2.0])
                fw.append([np.abs(filter_size[m][1] + filter_size[m][0]) / 2.0])
            else:
                fc.append([0.0])
        ...
        if isinstance(filter_size, (list, tuple)):
            fc = [(filter_size[1] - filter_size[0]) / 2.0]
            fw = [np.abs(filter_size[1] + filter_size[0]) / 2.0]
        else:
            fc = [0.0]
            fw = [filter_size]
    return fourier_filter(
        ...
        add_clean_residual=add_clean_residual,
    )
...
def dayenu_filter(
    ...
    if not (d_dim == 1 or d_dim == 2):
        raise ValueError(
            "number of dimensions in data array does not "
            "equal 1 or 2! data dim = %d" % (d_dim)
        )
    if not (w_dim == 1 or w_dim == 2):
        raise ValueError(
            "number of dimensions in wgts array does not "
            "equal 1 or 2! wght dim = %d" % (w_dim)
        )
    if not w_dim == d_dim:
        raise ValueError(
            "number of dimensions in data array does not equal "
    ...
    for dim in range(d_dim):
        if not d_shape[dim] == w_shape[dim]:
            raise ValueError(
                "number of elements along data dimension %d, nel=%d"
    ...
    if 0 in filter_dimensions and 1 in filter_dimensions:
        for avar, aname in zip(check_vars, check_names):
            err_msg = (
                "2d clean specified! %s must be a length-2 list of lists for 2d clean"
                % aname
            )
        ...
        if not len(x) == 2:
            raise ValueError(
                "For 2d filtering, x must be 2d long list or tuple or ndarray"
            )
        for j in range(2):
        ...
        for aname1, avar1 in zip(check_names, check_vars):
            for aname2, avar2 in zip(check_names, check_vars):
                if not len(avar1[fs]) == len(avar2[fs]):
                    raise ValueError(
                        "Number of elements in %s-%d must equal the"
    ...
    for fs in range(2):
        ...
        info["filter_params"]["axis_%d" % fs][
            "filter_half_widths"
        ] = filter_half_widths[fs]
    ...
    for ff in filter_factors:
        for fv in ff:
            if fv <= 0.0:
                raise ValueError(
                    "All filter factors must be greater than zero! You provided %.2e :(!"
                    % (fv)
                )
    ...
    for fs in filter_dimensions:
        ...
        for sample_num, sample, wght in zip(range(data.shape[fs - 1]), _d, _w):
            filter_key = _fourier_filter_hash(
                filter_centers=filter_centers[fs],
                filter_half_widths=filter_half_widths[fs],
                filter_factors=filter_factors[fs],
                x=x[fs],
                w=wght,
            ...
            if not filter_key in cache:
                ...
                if (
                    np.count_nonzero(wght) / len(wght) >= skip_wgt
    ...
    if zero_residual_flags:
        output = output * (~np.isclose(wgts, 0.0, atol=1e-10)).astype(float)
...
def wedge_filter(
    ...
    **kwargs,
    ...
    return delay_filter(
        sdf=sdf, data=data, wgts=wgts, max_dly=bl_dly, skip_wgt=skip_wgt, **kwargs
    )


def delay_filter(data, wgts, max_dly, sdf, skip_wgt=0.5, mode="clean", **kwargs):
    """Apply a wideband delay filter to data. Variable names preserved for
        backward compatability with capo/PAPER analysis.
...
def fringe_filter(data, wgts, max_frate, dt, skip_wgt=0.5, mode="clean", **kwargs):
    ...
    if isinstance(max_frate, (list, tuple)):
        fc = (max_frate[1] - max_frate[0]) / 2.0
        fw = np.abs(max_frate[1] + max_frate[0]) / 2.0
    else:
        fc = 0.0
        fw = max_frate
    return fourier_filter(
...
def vis_filter(
    ...
    # print deprecation warning
    warn(
        "Warning: dspec.vis_filter will soon be deprecated in favor of filtering.vis_filter",
        DeprecationWarning,
    )
    ...
    if dt is not None or max_frate is not None:
        timeclean = True
        assert (
            max_frate is not None and dt is not None
        ), "Must specify both max_frate and dt for time cleaning"

    freqclean = False
    if sdf is not None or bl_len is not None:
        freqclean = True
        assert (
            sdf is not None and bl_len is not None
        ), "Must specify both bl_len and sdf for frequency cleaning"
    ...
    # 2D clean
        ...
        # get bl delay
        bl_dly = _get_bl_dly(
            bl_len, horizon=horizon, standoff=standoff, min_dly=min_dly
        )
        freqs = np.arange(data.shape[1]) * sdf
        times = np.arange(data.shape[0]) * dt
        if isinstance(max_frate, (list, tuple)):
        ...
        if isinstance(window, str):
            window = (window, window)
        mdl, res, info = fourier_filter(
            data=data,
            wgts=wgts,
            x=[times, freqs],
            filter_centers=[[fc], [0.0]],
            filter_half_widths=[[fw], [bl_dly]],
            filter_dims=[1, 0],
            tol=tol,
            window=window,
            edgecut_low=edgecut_low,
...
def _get_bl_dly(bl_len, horizon=1.0, standoff=0.0, min_dly=0.0):
...
def gen_window(
    window, N, alpha=0.5, edgecut_low=0, edgecut_hi=0, normalization=None, **kwargs
):
    """
    ...
    if Ncut >= N:
        raise ValueError(
            f"Ncut >= N for edgecut_low {edgecut_low} and edgecut_hi {edgecut_hi}"
        )
    if edgecut_hi > 0:
        edgecut_hi = -edgecut_hi
    else:
    ...
    if normalization == "rms":
        w /= np.sqrt(np.mean(np.abs(w) ** 2.0))
...
def fourier_operator(dsize, nmax, nmin=None, L=None):
    ...
    F = np.array([np.exp(-1.0j * _n * nu / L) for _n in n])
    return F
...
def delay_filter_leastsq_1d(
    ...
    if isinstance(nmax, tuple) or isinstance(nmax, list):
        nmin = nmax[0]
        nmax = nmax[1]
        assert isinstance(nmin, int) and isinstance(
            nmax, int
        ), "Provide integers for nmax and nmin"
    elif isinstance(nmax, int):
        nmin = -nmax
    if operator is None:
        F = fourier_operator(
            dsize=data.size, nmin=nmin, nmax=nmax, L=fundamental_period
        )
    else:
        F = operator
        cshape = nmax - nmin + 1
        if F.shape[0] != cshape:
            raise ValueError(
                "Fourier basis operator has the wrong shape. "
    ...
    if use_linear:
        ...
        res = lsq_linear(A / mat_sigma**2.0, w * data / sigma**2.0)
        cn_out = res.x
    else:
    ...
    if add_noise:
        noise = np.random.randn(np.sum(flags)) + 1.0j * np.random.randn(np.sum(flags))
        if isinstance(sigma, np.ndarray):
...
def delay_filter_leastsq(
    ...
    if operator is None:
        F = fourier_operator(
            dsize=data.shape[1], nmax=nmax, nmin=nmin, L=fundamental_period
        )
    else:
...
def _fit_basis_1d(
        ...
        if basis.lower() == "dft":
            suppression_vector = np.hstack(
                [
                    ...
                    - sf
                    * np.ones(
                        2 * int(np.ceil(fw * basis_options["fundamental_period"]))
                    )
    ...
    if method == "leastsq":
        ...
        fm_key = _fourier_filter_hash(
            filter_centers=filter_centers,
            filter_half_widths=filter_half_widths,
            filter_factors=suppression_vector,
            x=x,
            w=w,
            hash_decimal=hash_decimal,
            label="fitting matrix",
            basis=basis,
        )
        if basis.lower() == "dft":
        ...
        raise ValueError(
            "Provided 'method', '%s', is not in ['leastsq', 'matrix']." % (method)
        )
    model = amat @ (suppression_vector * cn_out)
    resid = (y - model) * (~np.isclose(w, 0, atol=1e-10)).astype(
        float
    )  # suppress flagged residuals (such as RFI)
...
def _clean_filter(
    ...
    if not clean2d:
        ...
        filter_centers = [[0.0], copy.deepcopy(filter_centers)]
    ...
    window = [
        gen_window(
            ...
            data.shape[m],
            alpha=alpha,
            edgecut_low=edgecut_low[m],
            edgecut_hi=edgecut_hi[m],
        )
        for m in range(2)
    ]
    ...
    if filt2d_mode == "rect" or not clean2d:
        ...
        for fc0, fw0 in zip(filter_centers[0], filter_half_widths[0]):
            for fc1, fw1 in zip(filter_centers[1], filter_half_widths[1]):
                ...
                area += area_temp
        area = (area > 0.0).astype(int)
    else:
        raise ValueError(
            "%s is not a valid filt2d_mode! choose from ['rect', 'plus']"
            % (filt2d_mode)
        )
    if clean2d:
    ...
    if not clean2d:
        for i, _d, _w, _a in zip(
            np.arange(_data.shape[0]).astype(int), _data, _wgts, area
        ):
            ...
            if _w[0] < skip_wgt or np.all(np.isclose(_d, 0.0)):
                _d_cl[i] = 0.0
                _d_res[i] = _d
                info["status"]["axis_1"][i] = "skipped"
            else:
                _d_cl[i], _info = aipy.deconv.clean(
                    _d,
                    _w,
                    area=_a,
                    tol=tol,
        ...
        if not np.all(np.isclose(_data, 0.0)) and np.abs(_wgts).max() > skip_wgt:
            _d_cl, _info = aipy.deconv.clean(
                _data,
                _wgts,
                area=area,
                tol=tol,
...
def _fit_basis_2d(
    ...
    if not filter2d:
        ...
        for k in basis_options:
            if (
                not isinstance(basis_options[k], (tuple, list))
                or not len(basis_options[k]) == 2
            ):
                raise ValueError(
                    "basis_options values must be 2-tuple or 2-list for 2d filtering."
                )
        basis_options = [
            {k: basis_options[k][0] for k in basis_options},
            {k: basis_options[k][1] for k in basis_options},
        ]
    # filter -1 dimension
    ...
    for (
        ...
        _y,
        _w,
    ) in zip(range(data.shape[0]), data, wgts):
        if (
            np.count_nonzero(_w) / len(_w) >= skip_wgt
            and np.count_nonzero(_w[:max_contiguous_edge_flags]) > 0
            and np.count_nonzero(_w[-max_contiguous_edge_flags:]) > 0
        ):
            model[i], _, info_t = _fit_basis_1d(
                x=x[1],
                y=_y,
                w=_w,
    ...
    if np.any(
        ...
        info["filter_params"]["axis_1"]["filter_half_widths"] = info_t[
            "filter_half_widths"
        ]
        info["filter_params"]["axis_1"]["suppression_factors"] = info_t[
            "suppression_factors"
        ]
    ...
    if filter2d:
        ...
        for (
            ...
            _y,
            _w,
        ) in zip(range(model.shape[1]), model.T, wgts_time.T):
            if (
                np.count_nonzero(_w) / len(_w) >= skip_wgt
                and np.count_nonzero(_w[:max_contiguous_edge_flags]) > 0
                and np.count_nonzero(_w[-max_contiguous_edge_flags:]) > 0
            ):
                model.T[i], _, info_t = _fit_basis_1d(
                    x=x[0],
                    y=_y,
                    w=_w,
                ...
                info["status"]["axis_0"][i] = "skipped"
        if np.any(
            [info["status"]["axis_0"][i] == "success" for i in info["status"]["axis_0"]]
        ):
            ...
            info["filter_params"]["axis_0"]["filter_half_widths"] = info_t[
                "filter_half_widths"
            ]
            info["filter_params"]["axis_0"]["suppression_factors"] = info_t[
                "suppression_factors"
            ]
    ...
    if filter_dims[0] == 0:
        ...
        for k in info:
            ...
            info[k]["axis_1"] = {}
    if zero_residual_flags:
        residual = residual * (~np.isclose(wgts, 0.0, atol=1e-10)).astype(
            float
        )  # set residual to zero in flags.
    return model, residual, info


def fit_solution_matrix(
    weights, design_matrix, cache=None, hash_decimal=10, fit_mat_key=None
):
    """
    ...
    if fit_mat_key is None:
        opkey = (
            ("fitting_matrix",)
            + tuple(np.round(weights.flatten(), hash_decimal))
    ...
    if not opkey in cache:
        ...
        if np.linalg.cond(cmat) >= 1e9:
            warn(
                "Warning!!!!: Poorly conditioned matrix! Your linear inpainting IS WRONG!"
            )
...
def dpss_operator(
    ...
    crit_provided_name = [
        label for m, label in enumerate(crit_labels) if crit_provided[m]
    ]
    ...
    opkey = _fourier_filter_hash(
        ...
        filter_half_widths=filter_half_widths,
        filter_factors=[0.0],
        crit_name=crit_provided_name[0],
        x=x,
        w=None,
        hash_decimal=hash_decimal,
        label="dpss_operator",
        crit_val=tuple(crit_provided_value[0]),
    )
    if not opkey in cache:
        ...
        x, _, _, inserted = place_data_on_uniform_grid(
        ...
        if nterms is None:
            ...
            for fn, fw in enumerate(filter_half_widths):
                if not eigenval_cutoff is None:
                    ...
                    Nw = 2 * nf * df * fw + 2 / np.pi**2 * np.log(4 * nf) * np.log(
                        4 / (eigenval_cutoff[fn] * (1 - eigenval_cutoff[fn]))
                    )
                    dpss_vectors, eigvals = windows.dpss(
                        nf, nf * df * fw, int(min(Nw, nf)), return_ratios=True
                    )
                ...
                if not edge_suppression is None:
                    ...
                    rms_residuals = np.asarray(
                        [
                            np.sqrt(
                                np.mean(
                                    np.abs(
                                        edge_tone - np.sum(fit_components[:k], axis=0)
                                    )
                                    ** 2.0
                                )
                            )
                    ...
                    nterms.append(
                        np.max(np.where(rms_residuals >= edge_suppression[fn]))
                    )
                if not avg_suppression is None:
                    dpss_vectors = windows.dpss(nf, nf * df * fw, nf)
                    sinc_vector = np.sinc(2 * fw * df * (np.arange(nf) - nf / 2.0))
                    sinc_vector = sinc_vector / np.sqrt(np.mean(sinc_vector**2.0))
                    ...
                    rms_residuals = np.asarray(
                        [
                            np.sqrt(
                                np.mean(
                                    np.abs(
                                        sinc_vector - np.sum(fit_components[:k], axis=0)
                                    )
                                    ** 2.0
                                )
                            )
                    ...
                    nterms.append(
                        np.max(np.where(rms_residuals >= avg_suppression[fn]))
                    )
        # next, construct A matrix.
        amat = []
        for fc, fw, nt in zip(filter_centers, filter_half_widths, nterms):
            amat.append(
                np.exp(2j * np.pi * (yg[:, :nt] - xc) * fc)
                * windows.dpss(nf, nf * df * fw, nt).T
            )
        if len(amat) > 1:
            amat = np.hstack(amat)
        else:
...
def delay_interpolation_matrix(
    nchan, ndelay, wgts, fundamental_period=None, cache=None, window="none"
):
    """
    ...
    if not np.sum((np.abs(wgts) > 0.0).astype(float)) >= 2 * ndelay:
        raise ValueError(
            "number of unflagged channels must be greater then or equal to number of delays"
        )
    matkey = (nchan, ndelay, fundamental_period) + tuple(wgts)
    amat = dft_operator(
        x=np.arange(nchan) - nchan / 2.0,
        filter_centers=[0.0],
        filter_half_widths=[ndelay / fundamental_period],
        cache=cache,
        fundamental_period=fundamental_period,
    )
...
def dayenu_mat_inv(
    ...
    filter_key = _fourier_filter_hash(
        ...
        w=None,
        hash_decimal=hash_decimal,
        label="dayenu_matrix_inverse",
        wrap=wrap,
        wrap_interval=wrap_interval,
        nwraps=nwraps,
        ...
        for fc, fw, ff in zip(filter_centers, filter_half_widths, filter_factors):
            if not ff == 0:
                if not wrap:
                    sdwi_mat = (
                        sdwi_mat
                        + np.sinc(2.0 * (fx - fy) * fw).astype(np.complex128)
                        * np.exp(-2j * np.pi * (fx - fy) * fc)
                        / ff
                    )
                else:
                    ...
                    for wnum in np.arange(-nwraps // 2, nwraps // 2):
                        ...
                        sdwi_mat = (
                            sdwi_mat
                            + np.sinc(2.0 * (fx - fy - offset) * fw).astype(
                                np.complex128
                            )
                            * np.exp(-2j * np.pi * (fx - fy - offset) * fc)
                            / ff
                        )
