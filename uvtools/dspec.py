...
def high_pass_fourier_filter(data, wgts, filter_size, real_delta, twoD=False, tol=1e-9, window='none', 
    ...
    Arguments:
        ...
        filter_size: float, tuple, or list of tuples
            The size of the filter in fourier space that is filtered out,
            in units of 1/[real_delta]. For 1D clean, feeding as a float implies a symmetric filter
            with filter_size as the half-width.
            Feeding as a tuple specifies the lower and upper bounds of the filter.
            For a 2D clean, filter_size must be a len-2 list, and the elements
            of the list follow the same rules for the 1D clean described above.
        real_delta: the bin width in real space of the dimension to be filtered.
            If twoD, then real_delta must also be a list.
        twoD : bool, if True perform 2D clean, else perform a 1D clean on last axis.
    ...
    if twoD:
        ...
        assert isinstance(real_delta, (int, np.integer, float, np.float)), "if not twoD, real_delta must be a float"
    ...
    # 2D clean on 2D data
        ...
        # setup _d and _w arrays
        _window = np.empty(data.shape[0])
        _window[:] = aipy.dsp.gen_window(data.shape[0], window=window, **win_kwargs)
        _window = _window[:, None]
        _d = np.fft.ifft(data * wgts * _window, axis=0)
        _w = np.fft.ifft(wgts * _window, axis=0)
        _window = np.empty(data.shape[1])
        _window[:] = aipy.dsp.gen_window(data.shape[1], window=window, **win_kwargs)
        _window = _window[None, :]
        _d = np.fft.ifft(_d * _window, axis=1)
        _w = np.fft.ifft(_w * _window, axis=1)
        ...
        # run clean
        _d_cl, info = aipy.deconv.clean(_d, _w, area=area, tol=tol, stop_if_div=False, maxiter=maxiter, gain=gain)
        d_mdl = np.fft.fft(np.fft.fft(_d_cl, axis=0), axis=1)
        del info['res']
...
def fringe_filter(data, wgts, max_frq, dt, tol=1e-4, skip_wgt=0.5, maxiter=100,
    ...
    Args:
        ...
        max_frq : float, maximum frequency [1/sec] to CLEAN
    ...
    """
    # run fourier filter
    mdl, res, info = high_pass_fourier_filter(data.T, wgts.T, max_frq, dt, tol=tol, window=window,
                                              skip_wgt=skip_wgt, maxiter=maxiter, gain=gain, **win_kwargs)
    return mdl.T, res.T, info


def vis_filter(data, wgts, max_frq=None, dt=None, bl_len=None, sdf=None, standoff=0.0, horizon=1., min_dly=0., 
    ...
    if both bl_len & sdf and max_frq & dt variables are specified.

    Args:
        data : 1D or 2D data array. If 2D has shape=(Ntimes, Nfreqs)
        wgts : float weight array, matching shape of data
        max_frq : float, maximum frequency [1/sec] to CLEAN along time axis.
        dt : float, time-bin width [sec]
        bl_len: length of baseline (in 1/[sdf], typically ns)
        sdf: frequency channel width (typically in GHz)
        standoff: fixed additional delay beyond the horizon (same units as bl_len)
    ...
    # type checks
    timeclean = False
    if dt is not None or max_frq is not None:
        timeclean = True
        assert max_frq is not None and dt is not None, "Must specify both max_frq and dt"
    ...
    if not twoD:
        # time clean
        if timeclean:
            mdl, res, info = fringe_filter(data, wgts, max_frq, dt, tol=tol, skip_wgt=skip_wgt, maxiter=maxiter,
    ...
    # 2D clean
        ...
        # 2D clean
        mdl, res, info = high_pass_fourier_filter(data, wgts, (max_frq, bl_dly), (dt, sdf), tol=tol, window=window,
                                                  maxiter=maxiter, gain=gain, twoD=True, **win_kwargs)
