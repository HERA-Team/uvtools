...
o.add_option(
    "-u", "--unmask", dest="unmask", action="store_true", help="Plot masked data, too."
)
o.add_option(
    "-o",
    "--out_file",
...
o.add_option(
    "--nolegend", dest="nolegend", action="store_true", help="Omit legend in last plot."
)
o.add_option(
    "--share", dest="share", action="store_true", help="Share plots in a single frame."
)
o.add_option(
    "--xlim", dest="xlim", help="Limits on the x axis (channel/delay) for plotting."
)
o.add_option(
    "--ylim", dest="ylim", help="Limits on the x axis (time/delay-rate) for plotting."
)
o.add_option(
    "--plot_each",
    dest="plot_each",
...
def sort_func(a, b):
    ai, aj, pa = a
    bi, bj, pb = b
    if bi > ai or (bi == ai and bj > aj) or (bi == ai and bj == aj and pb > pa):
        return -1
    return 1
...
share = opts.share and not (
    is_chan_range and is_time_range
)  # disallow shared waterfalls
...
for cnt, bl in enumerate(bls):
    ...
    if is_chan_range and is_time_range:  # Need to plot a waterfall
        ...
        plots[cnt + 1] = uvtools.plot.waterfall(
            d,
            extent=(f1, f2, t2, t1),
            cmap=cmap,
            mode=opts.mode,
